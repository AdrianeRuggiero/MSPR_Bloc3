...
sys.path.append(str(Path(__file__).resolve().parent.parent))  # pour import racine
...
MODEL_PATH = "models/saved_models/best_model_mlp.pkl"
...
for epoch in range(1, 11):
    model = MLPClassifier(hidden_layer_sizes=(100,), max_iter=300, random_state=42 + epoch)
...
print(f" Modèle MLP sauvegardé dans {MODEL_PATH}")

# === Graphe accuracy
plt.figure()
plt.plot(range(1, 11), accuracies, marker='o')
plt.title("Évolution de l'Accuracy - MLP")
plt.xlabel("Époque")
plt.ylabel("Accuracy")
plt.grid(True)
plt.tight_layout()
...
# Camembert
counts.plot.pie(autopct='%1.1f%%', figsize=(6, 6), title="Prédiction Rhône - MLP")
plt.ylabel("")
plt.tight_layout()
plt.show()
